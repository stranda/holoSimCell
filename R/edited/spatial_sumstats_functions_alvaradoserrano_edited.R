...
stats.AFS = function(data, sstype, nsamples, pops.xy){
  ...
  for (I in 1:nrow(AFS.fol)){
    ...
    #!# Using the hist function was somewhat unreliable.  Doesn't give nsamples # of breaks in all cases!!
  ...
  #!# I'm not sure what these are supposed to be calculating
  #but it's not calculating the mean freq of derived alleles 
  #Taking a crack at calculating that with the modified versions
...
#ALL THREE OF THE OPTIONS BELOW WORK FOR THE MEAN ALLELE COUNT
#SD OPTIONS GIVE SIMILAR< THOUGH NOT IDENTICAL VALUES
#MAY PREFER RECONSTITUTING THE DATASET... IT'S CLEAN!
    
    #Weighted mean calculation
    #AFS.unistats[x,1] = weighted.mean(x = seq(ncol(AFS.fol))-1, w = AFS.fol[x,]/sum(AFS.fol[x,]))
    #AFS.unistats[x,1] = sum((AFS.fol[x,]/sum(AFS.fol[x,])) * (seq(ncol(AFS.fol))-1))
    #Weighted sd calclation - this is close to what I get if I reconstitute the dataset below, not exact though!
    #AFS.unistats[x,2] = sqrt(sum((AFS.fol[x,]/sum(AFS.fol[x,])) * ((seq(ncol(AFS.fol))-1)-AFS.unistats[x,1])^2))
...
sPCA.dist = function(data, pops.xy, nsamples, cpos=2, cneg=2, plot=T){
  ...
  #!# This repeat function messes up the order of individuals in the population
  #!# Need to find a better way to create this object - JDR 8/20/19
  #XY = data.frame(pop=rep(as.character(unique(data[,1])),nsamples),y=rep(pops.xy[,2],nsamples), x= rep(pops.xy[,3],nsamples))
  #I think I like this verion better, it's a loop, but a short one!
...
#!# This stuff isn't relevant anymore, pop IDs aren't numeric
