...
####
    ...
    names(Nvec) <- paste0("gen",seq(0,maxtime,samptime))

    npop <- length(Nvec)

    popslst <- vector("list",length=xdim*ydim)
...
###!# changed  slightly... fill in a column of Nvec_hist every samptime generations
        if(samptime > 0) {
            if(!(gen %% samptime)) 
            {
                                        #print(gen)
...
###        extFUN is a function passed as a getpophist parameter
        ...
        if (samptime>0) #if recordin history and population size goes to zero
        {
            extinct <- which(Nvec_hist[,paste0("gen",gen-1)]>0 & Nvec_hist[,paste0("gen",gen)]==0)  # if it is zero now AND was greater than zero last gen
        } else extinct <- which(Nvec == 0)
